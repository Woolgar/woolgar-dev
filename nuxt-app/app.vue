<template>
  <div class="container">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css2?family=Montserrat&display=swap" rel="stylesheet">
    <nav class="navbar navbar-expand-lg">
      <div class="container-fluid">
        <a class="navbar-brand" href="https://www.singaporeair.com/"><span><svg xmlns="http://www.w3.org/2000/svg"
              width="124" height="46" viewBox="0 0 124 46">
              <g fill-rule="evenodd" clip-rule="evenodd">
                <path fill="#fff"
                  d="M43.223 30.547h1.835v7.266h4.478v1.557h-6.313v-8.823zM50.945 30.547h1.82v8.823h-1.82v-8.823zM28.6 30.547h1.805v8.823H28.6v-8.823zM59.196 18.14c2.656 0 4.8 2.16 4.8 4.816 0 2.643-2.144 4.8-4.8 4.8-2.657 0-4.816-2.157-4.816-4.8 0-2.657 2.16-4.815 4.816-4.815zM65.787 30.547h6.915V32h-5.08v2.157h4.317v1.602h-4.317v2.14h5.08v1.47h-6.916v-8.823zM75.522 18.684h6.915v1.454h-5.08v2.143h4.33v1.6h-4.33v2.144h5.08v1.454h-6.915v-8.794zM33.21 39.37v-8.823h5.24s2.526.043 2.526 2.568c0 0 .234 2.1-1.91 2.95l2.658 3.305h-2.26l-3.67-4.58H37.7s1.365-.03 1.365-1.41c0-1.307-1.102-1.41-1.44-1.41h-2.583v7.4H33.21zM65.465 27.478v-8.794h5.24s2.526.044 2.526 2.57c0 0 .22 2.084-1.907 2.936l2.656 3.288h-2.26l-3.672-4.55h1.908s1.366-.03 1.366-1.41c0-1.32-1.102-1.41-1.44-1.41H67.3v7.37h-1.835zM45.615 27.478v-8.794h5.14s2.568.176 2.568 2.73c0 2.54-1.865 3.128-2.79 3.128h-2.07l-.983-1.337v4.272h-1.865zM16.958 39.37l4.198-8.823h1.997l4.125 8.823h-2.026l-1.116-2.585H20.07l-1.145 2.585h-1.967zM34.15 27.478l4.183-8.794h2.012l4.125 8.794h-2.04l-1.1-2.57H37.26l-1.145 2.57H34.15zM28.732 25.95h2.48v-4.036h1.762v5.563H28.35c-.69 0-4.023-.778-4.052-4.286-.03-3.478 3.01-4.507 4.185-4.507h4.537v1.557h-4.096s-2.746 0-2.746 2.892c0 2.246 2.275 2.82 2.554 2.82zM55.158 30.547h1.85v1.364l1.16-1.363h2.188s2.994-.163 2.994 3.038v5.785h-1.864v-6.063s-.06-1.204-1.32-1.204h-3.158v7.268h-1.85v-8.823zM14.432 18.684h1.835v1.35l1.16-1.35h2.187s3.01-.16 3.01 3.04v5.754H20.76v-6.033s-.073-1.204-1.336-1.204h-3.156v7.238h-1.835v-8.794zM10.086 18.684h1.806v8.794h-1.806v-8.794zM74.715 39.355v-1.498h5.108s1.013.19 1.013-1.042-.763-1.1-1.042-1.1h-2.95s-2.276.16-2.276-2.512c0-2.657 2.246-2.657 2.246-2.657h5.608v1.556h-4.918s-1.07 0-1.07 1.013.718 1.204 1.394 1.204h2.628s2.217-.236 2.217 2.407-2.275 2.628-2.275 2.628h-5.683zM.19 27.478V25.98H5.3s1.013.192 1.013-1.04-.763-1.103-1.042-1.103H2.32S.043 24 .043 21.342c0-2.673 2.246-2.658 2.246-2.658h5.608v1.557H2.98s-1.072 0-1.072 1.014.734 1.204 1.395 1.204H5.93s2.218-.234 2.218 2.393c0 2.645-2.276 2.63-2.276 2.63H.192v-.002z">
                </path>
                <path fill="#F99F1C"
                  d="M0 43.084h85.843L84.14 45.74H0v-2.656zM101.48 41.88h-3.157l6.37-10.056s.956-2.085-.292-4.287l-8.882-14.71s-.777-1.366 0-2.673l2.628-3.936L108.57 23.25s2.687 3.567-.235 7.985L101.48 41.88zM109.1 38.797l3.875-5.902s1.13-2.436 4.052-2.436h4.11L124 34.98s-2.26-.234-3.274 1.38l-4.228 6.37s-1.38 2.98-5.065 2.98h-19.91L99.1 34.084s1.07-1.365-.426-3.625l-6.74-10.835s-1.658-1.6.06-4.347l1.968-2.98 9.367 15.18s1.657 2.086 0 4.582l-7.034 10.966h14.828s1.674.31 3.112-1.834l3.686-5.668s.837-1.908 2.687-2.686h-2.98s-1.557.058-2.746 1.91l-3.274 5.062s-1.19 2.145-3.816 2.145h-5.184l6.255-9.705s3.406-4.478 1.014-8.588L99.51 6.513s-.778-1.482.117-2.862l2.26-3.39 11.804 19.012s1.732 2.848 1.732 5.05c0 2.2-1.38 4.698-2.144 5.96l-5.197 8.105s-.587.777-.176 1.072c.425.292.836-.237 1.19-.663z">
                </path>
                <path fill="#232F67"
                  d="M59.196 19.902c1.674 0 3.054 1.365 3.054 3.054 0 1.66-1.38 3.04-3.054 3.04-1.688 0-3.054-1.38-3.054-3.04 0-1.688 1.366-3.054 3.054-3.054zM47.45 22.927v-2.893h2.453s1.645-.088 1.645 1.483c0 1.38-1.336 1.41-1.645 1.41H47.45zM40.683 23.396l-1.366-3.126-1.395 3.126h2.76zM23.49 35.288l-1.35-3.156-1.395 3.156h2.745z">
                </path>
              </g>
            </svg></span>
        </a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent"
          aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
          <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          <ul class="navbar-nav me-auto mb-2 mb-lg-0">
            <li class="nav-item">
              <a class="nav-link active" aria-current="page" href="#">City Guide</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="#">inspiration</a>
            </li>
            <li class="nav-item">
              <a class="nav-link">Airline</a>
            </li>
          </ul>
        </div>
      </div>
    </nav>
    <div class="row">
      <div class="col-12">
        <img class="img-fluid" src="assets/images/hero-image.jpg" alt="" />
      </div>
      <h1 class="col-12 text-center py-4">INSIDER'S GUIDE <br />
        to Chiang Mai, Thailand</h1>
      <p>The second biggest city in Thailand after Bangkok, but not as busy and
        crowded. The northern part of Thailand is pleasant to visit all year
        around, but most recommended to go during the dry and cold season
        (Beginning of December until February). The city is perfect for historical
        tours, adventurous trips, or even calm and chill days. Here are some
        spots in Chiang Mai worth visiting.
      </p>
      <hr>
      <h2 class="text-center py-4">Ethical Elephant sanctuary</h2>
      <div class="col-6">
        <img class="img-fluid" src="assets/images/elephants.jpg" alt="" />

      </div>
      <div class="col-6">
        <p>
          <a class="sa-link" href="https://www.elephantnaturepark.org/">
            Elephant Nature Park
          </a>
          is a place that
          has rescued more than 75 elephants
          and other animals that had been
          suffering from abuse and starvation.
          Here, all animals are free from chains
          to roam all around. They offer a
          chance for visitors to feed, bathe, walk
          and interact with elephants in the
          sanctuary! You can either do this in
          one day or stay overnight to spend
          more time with them as well
        </p>
      </div>
    </div>
    <div class="row py-4">
      <div class="col-12 ">
        <h2 class="text-center">Wat Phra That Doi Suthep</h2>
      </div>
    </div>
    <div class="row">
      <div class="col-6"> <img class="img-fluid" src="assets/images/golden-temple.jpg" alt="" />
      </div>
      <div class="col-6"> <img class="img-fluid" src="assets/images/golden-buddha.jpg" alt="" />
      </div>
      <p class="py-4">Challenge yourself with more than 300 steps to the top of Doi Suthep
        mountain, where one of the most important temples in Northern
        Thailand stands. Once you arrive at the temple ground, you will find a
        stunning golden chedi surrounded by 3 halls housing several Buddha
        images. It is open from 5 AM to 8 PM, a perfect spot to experience a
        panoramic view of Chiang Mai with both sunrise and sunset</p>
    </div>
    <hr class="py-4" />

    <div class="row text-center py-4">
      <h2>Taste a bowl of Khao Soi</h2>
      <div class="col-6">
        <p>Khao Soi, an award-winning
          dish can be found in various
          places across town. It is a
          bowl of curry, coconut milk,
          and egg noodles that comes
          with either chicken, pork, beef,
          or tofu. It is traditional to be
          served with crispy egg
          noodles on top, with a side of
          raw onion, pickled cabbage
          and chili oil. All tastes and
          textures combine in a bowl,
          this dish is unforgettable.</p>
      </div>
      <div class="col-6">
        <div class="col-6"> <img class="img-fluid" src="assets/images/khaosoi.jpg" alt="" />

        </div>
      </div>
    </div>
    <div class="row  py-4">
      <h2 class="text-center py-4">Nimmanhaemin Road</h2>
      <div class="card-group">
        <div class="col ">
          <img class="img-fluid" src="assets/images/coffee.jpg" alt="" />
        </div>
        <div class="col">
          <img class="img-fluid" src="assets/images/cafe.jpg" alt="" />
        </div>
        <div class="col">
          <img class="img-fluid" src="assets/images/dessert.jpg" alt="" />
        </div>
      </div>
    </div>

    <div class="row  py-4">
      <div class="col-12">
        <p>If you would like to try a low living for a day, Nimmanhaemin Road is your
          answer. It is a paradise for cafe hoppers both visitors and locals, located in
          the heart of Chiang Mai. There are large malls, stylish boutiques, street
          markets, restaurants, and numerous coffee stops. Besides that, when the
          night comes, you can try out several local bars with live music on the
          same road as well.</p>
      </div>
      <hr class=" py-4" />
      <div class="col-12">
        <h2 class="text-center  py-4">Thailand's tallest mountain</h2>
        <img class="img-fluid" src="assets/images/landscape.jpg" alt="" />
        <p class=" py-4">Doi Inthanon is located 90 minutes from Chiangmai. It is home to
          many spectacular waterfalls and landscapes. The national park there is
          a known place for locals to have an adventure for a day or camp up at
          night. If you are up early enough, there is also a chance for you to
          witness the forest hiding in a sea of mist in the morning!</p>
      </div>
      <div class="col-12 text-center  py-4">
        <h2>Stay at the old charming hotel</h2>
      </div>
      <div class="row">
        <div class="col-6">
          <img class="img-fluid" src="assets/images/hotel.png" alt="" />

        </div>
        <div class="col-6">
          <img class="img-fluid" src="assets/images/hotel-room.jpg" alt="" />

        </div>
        <p class=" py-4">The 137 Pillars Hotel Chiang Mai was a home of the Boneo Trading
          Company, from 1889 to 1958. This luxury property is today restored into a
          place for you to relax and enjoy a home-style residential concept
          surrounded by tropical gardens.</p>
      </div>
    </div>
  </div>
</template>


<script lang="ts" setup>

</script>


<style lang="scss">
body {
  margin: 0 !important;
  background-color: #f6f3ee;
}

h1 {
  color: #544541;
  font-size: 55px;
}

hr {
  color: #ad795b;
  max-width: 80% !important;
  height: 2px;
  margin: 0 auto;
}

h2,
h3,
h4,
h5,
h6,
p {
  color: #3d2f2b;
  font-family: 'Montserrat', sans-serif;
}

.container {
  max-width: 1000px;
}

.card-group {
  img {
    object-fit: cover;
    height: 100%;
    padding: 5px;
  }
}

nav {
  background-color: #00266b;

  a {
    color: #fff !important;
    text-decoration: none;
  }

}

.sa-link {
  color: #3d2f2b;
  font-weight: 600;
  text-decoration: underline;
}
</style>